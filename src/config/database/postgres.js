...
export function resolveSslConfig() {
    ...
    if (
        /sslmode=(require|verify-ca|verify-full|prefer)/i.test(
            url,
        )
    ) {
        return {
...
export const pgConfig = {
    ...
    options: {
        ...
        retries:
            parseInt(process.env.POSTGRES_RETRIES) ||
            3,

        backoffBase:
            parseInt(process.env.POSTGRES_BACKOFF_BASE) ||
            100,

        backoffMultiplier:
            parseInt(process.env.POSTGRES_BACKOFF_MULTIPLIER) ||
            2,
    },
